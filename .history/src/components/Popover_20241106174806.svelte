<script lang="ts">
	...
	const receiveTransition = (node: Element, params: CrossfadeParams & { key: any; }) => receive(node, { ...params, key: params.key });
</script>
...
/*************  ✨ Codeium Command 🌟  *************/
	<svg xmlns="http://www.w3.org/2000/svg" class="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor" in:receive={({ node, params }) => receiveTransition(node, { ...params, key: params.key })} out:send>
...
/******  31888cdf-af2f-440b-bda9-b81ce28a063a  *******/
		{#if showMenu}
			<path
				key="close"
