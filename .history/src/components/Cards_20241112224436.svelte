...
<section
	class="z-10 grid grid-rows-1 gap-8 pb-0 lg:grid-cols-2 md:gap-8 md:px-8 xl:grid-cols-3"
>
	{#each concepts as concept}
		...
		>
			...
			<div class="row-span-2 grid grid-cols-subgrid place-items-center items-center justify-center">
				<h2 class="] mt-[28px] text-xl font-semibold uppercase tracking-wide text-[--primary-header]">
					{concept.name}
				</h2>
			</div>

			<div
				class=" row-span-12 my-5 w-full grid grid-rows-subgrid place-items-start items-start gap-4 text-pretty border border-transparent bg-[--card-bg-color] px-4 pt-7 pb-6 text-center"
			>
				{#each concept.features as feature}
					<h3
						class="border-b-1 mx-auto  grid w-[35ch]  grid-rows-subgrid text-balance border-[--text-color] px-0 text-[14px] font-normal leading-snug tracking-wider text-sky-200 py-0 first:border-t-2 first:pt-6 last:pb-6 last:border-b-2 lg:text-[16px] border-s-fuchsia-400 ">
						{feature}
					</h3>
				{/each}
			</div>
