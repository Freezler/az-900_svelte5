...
>
	{#each concepts as concept}
		...
		>
			...
			<div
				class=" row-span-12 my-6 grid grid-rows-subgrid place-items-start items-start gap-4 text-pretty rounded-xl border border-transparent bg-sky-500/20 px-4 py-6 text-center"
			>
				{#each concept.features as feature}
					<h3
						class="border-b-1 mx-auto mb-0 grid w-[35ch]  grid-rows-subgrid text-balance border-[--text-color] px-0 text-[12px] font-normal leading-snug tracking-wider text-sky-200 last:pb-2 last: last:border-b-2 lg:text-[16px] border-s-fuchsia-400 ">
						{feature}
					</h3>
				{/each}
			</div>
			<button
				type="button"
				class="mx-auto mt-2 w-[220px] grid-cols-subgrid rounded-xl border-[2px_solid_var(--text-color)] bg-sky-900 px-8 py-4 text-base font-semibold uppercase text-[--text-color] transition duration-[420ms] hover:bg-sky-500 hover:text-white hover:shadow-[2px_2px_42px_rgb(65,183,255,0.9)] md:text-lg lg:text-lg"
