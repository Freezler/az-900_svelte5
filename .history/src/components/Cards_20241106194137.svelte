	...
	{#each plans as plan}
		...
		>
			...
			<div class="subgrid grid">
				<h2 class="subgrid text-xl grid font-bold text-white py-4">{plan.name}</h2>
			</div>
			<div class="subgrid grid-col-4 grid text-pretty  text-sm">
				{#each plan.features as feature}
					<p class="my-4 text-pretty text-white">{feature}</p>
				{/each}
			</div>
			<button
