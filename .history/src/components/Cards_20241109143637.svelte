...
<section class="py-8 grid gap-8 md:grid-cols-2 md:gap-8 md:px-8 xl:grid-cols-3 grid-rows-1">
	{#each concepts as concept}
		...
		>
			<img
				src={concept.image}
				class=" w-full h-[22vh] backdrop-[blur(2px)]  border-b-2 border-[--text-color] object-cover object-center"
				loading="lazy"
				alt="concept"
			/>
			...
			<div
				class="my-6  p-2 rounded-xl bg-sky-500/60 grid grid-rows-subgrid place-items-start items-start text-pretty text-center gap-4 row-span-12 "
			>
				{#each concept.features as feature}
					<p
						class="mx-auto w-[35ch] text-balance px-0  font-normal text-[--text-color]  text-sm  lg:text-[16px] border-b-2 border-[--text-color] leading-2 "
					>
						{feature}
					</p>
				{/each}
