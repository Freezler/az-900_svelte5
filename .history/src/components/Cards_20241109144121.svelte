	...
	{#each concepts as concept}
		...
		>
			...
			<div class="grid grid-cols-subgrid  place-items-center items-center justify-center">
				<h2
					class="text-xl uppercase  mt-6 font-semibold text-[--text-color]  "
				>
					{concept.name}
				</h2>
			</div>
			...
			<div
				class="my-6  p-4 rounded-xl bg-sky-500/30 backdrop-blur  grid grid-rows-subgrid place-items-start items-start text-pretty text-center gap-4 row-span-12 "
			>
				{#each concept.features as feature}
					<p
						class="mx-auto w-[35ch] text-balance px-0  font-normal text-sky-200  text-sm  lg:text-[16px] border-b-2 -border-spacing-16 border-[--text-color] leading-tight "
					>
						{feature}
					</p>
				{/each}
