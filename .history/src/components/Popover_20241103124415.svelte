...
<button
	on:click={toggle}
	tabindex="0"
	class=" z-100 fixed right-0 top-0 m-[22px] size-[85px] rounded-full border-4 border-yellow-400 text-6xl text-yellow-300 transition-all hover:bg-[hsl(210,55%,8%,0.4)]"
...
{#if isMenuOpen}
	<div
		class="z-100 fixed bottom-0 left-0 mt-auto h-[42vh] w-full bg-blue-500/80 text-center"
		id="menu"
		role="menu"
	...
	>
		<ul
			class="flex h-full w-full flex-col place-items-center items-center justify-center gap-4 text-[--bg-color] text-3xl"
		>
			<li role="none">
